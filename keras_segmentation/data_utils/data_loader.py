...
def get_pairs_from_paths(images_path, segs_path):
    ...
    for im in images:
        ...
        assert (seg in segmentations_d), \
               (im + " is present in " + images_path + " but " +
                seg_bnme + " is not found in " + segs_path +
                " . Make sure annotation image are in .png")
        ret.append((im, seg))

    return ret


def get_image_arr(path, width, height, imgNorm="sub_mean",
...
def verify_segmentation_dataset(images_path, segs_path, n_classes):
    ...
    for im_fn, seg_fn in tqdm(img_seg_pairs):
        ...
        assert (img.shape[0] == seg.shape[0] and img.shape[1] == seg.shape[1]
                ), "The size of image and the annotation does not" \
                   "match or they are corrupt " + im_fn + " " + seg_fn
        assert (np.max(seg[:, :, 0]) < n_classes), "The pixel values of seg" \
            " image should be from 0 to " + \
